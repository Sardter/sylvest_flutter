...
import 'package:sylvest_flutter/_extra_libs/chat_library/chat_types/flutter_chat_types.dart'
    as types;
import 'package:sylvest_flutter/_extra_libs/chat_library/chat_ui/src/widgets/community_message.dart';
import 'package:sylvest_flutter/_extra_libs/chat_library/chat_ui/src/widgets/post_message.dart';
...
class Message extends StatelessWidget {
  ...
  Widget _buildMessage() {
    switch (message.type) {
      ...
      case MessageType.Post:
        final postMessage = message as types.PostMessage;
        return PostMessage(message: postMessage);
      case MessageType.Community:
        final communityMessage = message as types.CommunityMessage;
        return CommunityMessage(message: communityMessage);
      default:
        return Container();
    }
  }
  ...
  Widget _buildSavedStatus(bool saved, context) {
    if (!saved) return SizedBox();
    return Icon(
      Icons.save,
      color: InheritedChatTheme.of(context).theme.primaryColor,
      size: 15,
  ...
  Widget _buildTime(bool currentUserIsAuthor, BuildContext context) {
    return Row(
      ...
      children: [
        _buildSavedStatus(message.saved, context),
        SizedBox(
          width: 5,
        ),
        Container(
          margin: EdgeInsets.only(
            right: currentUserIsAuthor ? 8 : 25,
          ),
          child: Text(
            ...
            GestureDetector(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  color:
                      !_currentUserIsAuthor || message.type == MessageType.Image
